...
test_numeric_log(const MunitParameter Parameters[], void *Fixture) {
    ...
    munit_assert_int(standard_log(pContext, NULL, &sParameters, &sResult, NULL), ==, ERROR_PARAMETER_COUNT);
    ...
    munit_assert_int(standard_log(pContext, NULL, &sParameters, &sResult, NULL), ==, ERROR_PARAMETER_TYPE);
    ...
    for(nType = 0; nType < 2; ++nType) {
        for(nIndex = 0; nIndex < 4; ++nIndex) {
            ...
            munit_assert_int(ll_insert(&sParameters, pParameter), ==, 0);
            munit_assert_int(standard_log(pContext, NULL, &sParameters, &sResult, NULL), ==, 0);
            munit_assert_uint32(sResult.Type, ==, pParameter->Value.Type);
            dValue = round(pValue->Result * 1e6) / 1e6;
